# 通知機能
入金シグナルが発生した時や、売買注文を実行した際には、登録したメールアドレス宛に通知メールを送信します。
...
- BALANCE_BUFFER=500              # 注文後に常に残しておく最低バッファ（JPY、推奨500～1000円）
- DEPOSIT_AMOUNT=10000            # 自動入金（1ループあたり最大）
...
# 売買判断ロジック
本ボットはRSI（相対力指数）とボリンジャーバンドを組み合わせて売買タイミングを判断します。
一般的に、RSIが30以下で「買い」、70以上で「売り」の目安とし、ボリンジャーバンドの上下限も参考にします。
