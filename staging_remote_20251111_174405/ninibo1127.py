...
# --- BEGIN compatibility shim inserted by helper (ensures AuthenticationError safe access) ---
try:
...
try:
    ...
    # If base_obj is not a proper module/class with .errors, replace with a minimal holder
    if base_obj is None or not hasattr(base_obj, 'errors') or isinstance(base_obj, (list, tuple, dict)):
        class _BaseErrors:
...
# --- BEGIN run_bot_di wrapper (added by helper) ---
def run_bot_di(dry_run=True, pair='BTC/JPY', interval_seconds=3600):
    """
    Safe DI entrypoint for testing and service wrappers.

    - dry_run: when True, sets DRY_RUN=1 if not already set.
    - pair, interval_seconds: passed to run_bot(...)
    Returns whatever run_bot returns (or raises).
    """
    import os, logging
    # Ensure DRY_RUN is set for this process (do not overwrite if already explicitly set)
    if dry_run and os.environ.get('DRY_RUN') is None:
        os.environ['DRY_RUN'] = '1'
    elif not dry_run and os.environ.get('DRY_RUN') is None:
        os.environ['DRY_RUN'] = '0'

    # Acquire logger if available
    try:
    ...
    if logger:
        logger.info("run_bot_di starting (dry_run=%s, pair=%s, interval=%s)", dry_run, pair, interval_seconds)
    else:
        print("run_bot_di starting (dry_run={}, pair={}, interval={})".format(dry_run, pair, interval_seconds))

    try:
        # call the existing run_bot entrypoint
        return run_bot(pair=pair, interval_seconds=interval_seconds)
    except Exception as exc:
        if logger:
            logger.exception("run_bot_di failed")
        else:
            print("run_bot_di failed:", exc)
        # Re-raise so the caller/test sees the failure
        raise
# --- END run_bot_di wrapper ---
